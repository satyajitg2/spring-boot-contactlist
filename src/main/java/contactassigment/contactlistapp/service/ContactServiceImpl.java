...
public class ContactServiceImpl implements ContactService {
	...
	public ContactDTO findByIdFetchOrganisation(Integer id) {
		...
		return (contact != null) ? ContactDTO.createByView(contact) : null;
	}
	...
	public List<ContactDTO> listByCriteriaFetchOrganisation(ContactSearchCriteriaDTO criteria) {
		...
		List<Contact> resultList = contactRepo.searchByNamesFetchOrganisation(criteria.getName(),
				criteria.getLastName(), criteria.getOrganisationName());

		return ContactDTO.createListBy(resultList);
	}
	
	@Override
	public List<ContactDTO> listByCriteriaFetchName(ContactSearchCriteriaDTO criteria)
	{
		List<Contact> resultList = contactRepo.searchByOrganisationNameFetchName(criteria.getOrganisationName());
	...
	@Override
	public List<ContactDTO> listByNameContainingIgnoreCase(ContactSearchCriteriaDTO criteria) {
		logger.debug("Query Criteria: " + criteria);

		List<Contact> resultList = contactRepo.findByNameContainingIgnoreCase(criteria.getName());

		return ContactDTO.createListBy(resultList);	
	}

	@Override
	public List<ContactDTO> listByLastnameContainingIgnoreCase(ContactSearchCriteriaDTO criteria) {
		logger.debug("Query Criteria: " + criteria);

		List<Contact> resultList = contactRepo.findByLastnameContainingIgnoreCase(criteria.getLastName());
